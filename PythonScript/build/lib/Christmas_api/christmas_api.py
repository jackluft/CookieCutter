...
class ChristmasAPI:
    ...
    def deleteChristmasWish(self, id: int):
        ...
        if sel.token != None:
            header = {"Authorization": f"Bearer {token}"}
            url = self.URL+"/christmas-list"
            response = requests.delete(url,headers=header,params=id)
            if response.status_code == 201:
    ...
    def addToWish(self, id: Int toys: List[str]):
        ...
        if self.token != None:
            ...
            response = requests.patch(url,headers=header,json=toys,params=id)
            return response.json()
        else:
            return "Token has not been Authericated. Please login with the login() function"
    ...
    def deleteToysFromWish(self,id: int, toys: List[str]):
        ...
        if self.token != None:
            ...
            response = requests.delete(url,headers=header,json=toys,params=id)
            return response.json()
        else:
            return "Token has not been Authericated. Please login with the login() function"
